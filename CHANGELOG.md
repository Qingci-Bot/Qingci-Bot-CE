...
### Added
- **CI 强化**：GitHub Actions 新增 `docker` 与 `install-script` 两个 job——`docker` 校验 compose 配置 + 构建镜像 + 容器运行冒烟（`/api/bot/health` 就绪判定，30s 超时）；`install-script` 校验 `bash -n` + 执行 `install.sh --dev`（SKIP_SYS_DEPS=1）+ `main.py` 启动冒烟（60s 超时）；为容器化与安装脚本提供自动化回归
