...
### Added
- **CI 迁移至 GitCode Actions**：CI 从 GitHub Actions（`.github/workflows/ci.yml`）迁移到 GitCode/AtomGit Action（`.gitcode/workflows/ci.yml`）——三个 job：`quality`（ruff lint + format + mypy + pytest + 覆盖率门槛，用官方 `checkout`/`setup-python` 插件）；`docker-build`（GitCode runner 无 Docker daemon，改用预装 **Kaniko** 无 daemon 构建 Dockerfile 并导出 tar 验证，含 `docker-compose` YAML 配置校验，替代原 daemon 构建 + 容器冒烟）；`install-script`（`bash -n` + `SKIP_SYS_DEPS=1 ./install.sh --dev` + `main.py` 启动冒烟，60s 超时）；`runs-on` 适配 `ubuntu-24` 三段式标签体系
