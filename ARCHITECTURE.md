...
2. **aiocqhttp** 收到 OneBot 11 事件，经 **v11\_compat**（`bot/core/v11_compat.py`）翻译为 **OneBot 12** 事件（`type`/`detail_type`/`message[]`）；**telegram 适配器**直接将原生更新归一化为 OneBot 12 事件（注入 `platform` 字段），统一分发至 **Dispatcher**
...
- **桌面（Electron 壳）**：Electron 桌面壳 spawn `python main.py --backend` — Python 仅作为内嵌后端（Bot+API），窗口/托盘/单实例/启动画面全由 Electron 承担；壳就绪时解析标准输出的 `\x1eQINGCI_READY <port>\x1e` 加载 Web UI
...
│       └── styles/            # 全局样式
├── desktop/
│   ├── electron/              # Electron 桌面壳（Electron-Python 混合架构的壳层）
│   │   ├── src/main.js        # 主进程：解析实例→spawn Python 后端→端口发现→单实例/托盘/启动画面/生命周期
│   │   ├── src/preload.js     # 预加载桥（contextBridge，暴露最小 IPC）
│   │   └── build/             # 壳内静态资源（boot.html 启动画面 / icon.png 托盘图标）
│   ├── py/                    # Python 侧桌面辅助（无 GUI，供 CLI/headless/main 复用）
│   │   ├── single_instance.py # 进程级单实例互斥（按数据根派生命名互斥量）
│   │   └── relaunch.py        # 跨进程重启助手（CLI/headless 切换/重命名实例后重启）
│   └── assets/
│       └── app-icon.ico       # 应用图标（Python exe 图标 + Electron 窗口/exe 图标）
├── scripts/                   # 运维/构建辅助脚本
...
| 平台接入  | 内核统一 OneBot 12 事件模型；输入含 OneBot 11（aiocqhttp 反向 WS，v11\_compat 翻译层）+ OneBot 12（原生反向 WS，事件直通）+ Telegram（Bot API 长轮询），统一 `PlatformAdapter` 契约 |
| LLM   | litellm (统一接口，延迟导入加速启动)                                                                                                                    |
| MCP   | mcp (Model Context Protocol，stdio/HTTP)                                                                                                    |
| 数据库   | SQLModel + Alembic + aiosqlite (WAL 模式)                                                                                                    |
| 定时任务  | APScheduler                                                                                                                                |
| 插件协议层 | qingci\_plugin\_sdk（PluginBase/Matcher/Rule/Permission，主项目薄转发）                                                                             |
| 前端    | Vue 3 + Vite + Pinia                                                                                                                       |
| 桌面    | Electron 壳（spawn Python `--backend`，加载 Web UI；托盘/单实例/启动画面于壳侧）                                                                              |
...
- **`base.py`**：定义 `PlatformAdapter` 契约（适配器名/展示名、启动与关闭、事件上报回调、发送消息、`get_status`/API 透传等），并提供 OneBot 高频群管理动作的便捷方法（`set_group_kick`/`set_group_ban`/`set_group_whole_ban`/`set_group_admin`/`set_group_card`/`set_group_name`/`get_group_member_list`/`get_group_member_info`，经 `_api_action` 映射平台动作名——v11 保持 `set_*` 命名、OneBot 12 映射为 `group.*` 点分命名）

- **`telegram.py`**：Telegram 平台实现——以 Bot API 长轮询（`getUpdates`）接入，由 `platforms.telegram.enabled/token/poll_interval` 控制；收到更新后归一化为 **OneBot 12 消息事件**并注入 `platform: "telegram"`。关键能力：① 群聊解析 `entities`（`mention` / `text_mention`）识别 `@Bot`，命中时写入 v12 `mention` 段（`user_id=self_id`）并置 `is_at_bot`，确保 at 触发模式在 Telegram 群聊生效（私聊由 SDK 规则天然放行）；② `photo` / `image/*` document 归一化为 `image` 段（`file_id`）+ `images`，`voice` → `voice`、`video` / `video_note` → `video` 段，消息 `sub_type` 语义对齐 OneBot（私聊 `friend`）；③ 发送消费 OneBot 12 标准段——`image` → `sendPhoto`、`voice` → `sendVoice`、`video` → `sendVideo`（file\_id / http(s) URL / `base64://` / `data:` / 本地路径，本地与 base64 走 multipart 上传，caption 附着首条媒体），`reply` 段 → 回复指定消息，其余不可渲染的段降级为纯文本并合并连续空白，之外走 `sendMessage`；④ `chat_member` / `my_chat_member` 成员变动归一化为 OneBot `notice`（`group_member_increase` / `group_member_decrease` / `group_admin_set` / `group_admin_unset`，被邀请 `sub_type=invite`），由既有事件 Matcher 消费；⑤ 轮询 offset 采用**整批确认**：先以 `max(update_id)+1` 推进游标再并发消费本批更新，单条处理失败也已被确认，避免失败更新无限重放
...
- 人格切换：`/persona` 命令会话级覆盖 system\_prompt
...
- 逐表迁移数据（messages / sessions / plugin\_configs / group\_configs / usage\_logs / audit\_logs）
...
1. 运行 Alembic 迁移重建表结构：

```bash
alembic upgrade head
```

1. 移除 SQLite 专用 PRAGMA 设置（`_set_sqlite_pragma` 函数与 `event.listen` 调用）。
