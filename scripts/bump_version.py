...
用法：
    python scripts/bump_version.py 1.7.0     # 同步升级三处版本号
    python scripts/bump_version.py --check   # 校验三处版本号是否一致（Commit 前自查用）
...
def main() -> int:
    ...
    if args.check:
        print(f"[一致] 三处版本号均为: {next(iter(distinct), '(未找到)')}")
        return 0

    assert args.version is not None
    if not apply_version(args.version):
        return 1
    print(f"[完成] 已将三处版本号统一升级为 {args.version}:")
    for name in ("pyproject.toml", "bot/__init__.py", "web/package.json"):
        print(f"  {name} ✓")
    return 0
