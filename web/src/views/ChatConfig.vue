...
const providerOptions = computed(() => {
  ...
  const keys = Object.keys(presets).length
    ? Object.keys(presets)
...
function resetForm() {
  ...
  Object.assign(form, {
    ...
    personas: Array.isArray(llm.personas) ? llm.personas.map((p) => ({ ...p })) : [],
...
function onProviderChange() {
  ...
  const isDefaultApiUrl =
    !form.api_url || Object.values(presets).some((p) => p.api_url && p.api_url === form.api_url);
  if (isDefaultApiUrl) {
  ...
  const isDefaultModel =
    !form.model || Object.values(presets).some((p) => p.model && p.model === form.model);
  if (isDefaultModel) {
      ...
      <div class="form-grid">
        ...
        <div class="form-group">
          ...
          <div class="model-actions">
            <button
              ...
              @click="fetchModels"
            >
              <span :class="{ spin: fetchingModels }">⟳</span>
              {{ fetchingModels ? '获取中' : '获取模型列表' }}
            </button>
            <span v-if="modelOptions.length" class="hint-text"
              >共 {{ modelOptions.length }} 个模型</span
            >
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group full-width">
          ...
          <textarea
            ...
            placeholder="定义 Bot 的人格、回复风格和知识边界..."
          />
        </div>
      </div>
    </div>
    ...
    <div class="card fade-in">
      ...
      <div v-for="(s, idx) in form.mcp_servers" :key="idx" class="persona-block">
        <div class="persona-row">
          ...
          <input
            v-model="s.command"
            placeholder="stdio 命令（如 npx / uvx / python）"
            class="mcp-cmd"
          />
        ...
        <input
          v-model="s.args"
          placeholder="命令参数，逗号分隔（如 -y, @modelcontextprotocol/server-filesystem, /tmp）"
          class="mcp-args"
        />
        <input
          v-model="s.url"
          placeholder="HTTP 模式地址（填写后忽略命令；如 http://localhost:8000/mcp）"
          class="mcp-args"
        />
      </div>
