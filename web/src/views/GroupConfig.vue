...
function onTriggerChange(group) {
  ...
  const payload =
    group.triggerValue === '' ? { trigger_mode: null } : { trigger_mode: group.triggerValue };
...
<template>
  ...
  <div class="page-body">
    <div class="card fade-in">
      ...
      <div class="hint-text">
        <strong>全局默认：</strong>{{ defaults.enabled === false ? '停用' : '启用' }} · 触发模式
        {{ defaults.trigger_mode || '（未设置）' }}。 触发模式选择"跟随全局"表示清除该群的独立设置。
      </div>
